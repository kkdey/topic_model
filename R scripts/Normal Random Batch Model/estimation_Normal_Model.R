...
omega_loglik = function(omega_vec,counts_vec,alpha,beta,lab)
{
...
while(iteration <= MaxIter)
{
	for(g in 1:G)
	{
		...
		fit=lmer(counts_col~omega0[,2]+omega0[,3]+omega0[,4]+(1|rand_col),
						REML=TRUE);
	...
	for(n in 1:dim(counts)[1])
	{
		...
		counts_vec=counts[n,];
		lab=Label.Batch[n];
		res=optim(reverse_transform(omega_vec), function(v) omega_loglik(transform(v),counts_vec,alpha,beta,lab) );
